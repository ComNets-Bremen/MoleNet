...
class Transceiver:
    ...
    def __init__(self,spi,cs,rst,busy,dio1):
        ...
        #self.write(bytes([_SET_DIO3_TXCO_CTRL, 0x02, 0x0, 0x02, 0x80])) #1.8 V for TXCO, delay: 640*15.625us=10ms  # # DIO3 TXCO control is disabled because this board does not use DIO3 to drive a TXCO; only DIO2 RF switch control is needed.
    ...
    def sleep(self, retain_config=True):
        """Put the transceiver into sleep mode.
        If retain_config is True, uses warm start (retains registers).
        If False, uses cold start (resets registers).
        """
        mode = 0x04 if retain_config else 0x00
        self.write(bytes([_SET_SLEEP, mode]))
        ...
        if (isinstance(msg,str) or isinstance(msg,bytes)) and (isinstance(freq,float) or isinstance(freq,int)):
            ...
            if blocking:
                ...
                while self.dio1.value() == 0:
                    ...
                    # 5-second safety timeout to prevent infinite hang
                    if utime.ticks_diff(utime.ticks_ms(), start_time) > 5000:
                        print("TX Timeout - Aborting")
                        tx_err = True
                        break
        ...
        for i in range(retries): #in case of error try again
            ...
            while True:
                ...
                if self.busy.value() == 0:
                    break
                if utime.ticks_diff(utime.ticks_ms(), _t0) > 500:
                    print("SX1262 BUSY timeout in write()")
                    break
            self.spi.write(msg)
            utime.sleep_ms(1)
    ...
    def read(self,buf, retries=3):
        ...
        while True:
                ...
                if self.busy.value() == 0:
                    break
                if utime.ticks_diff(utime.ticks_ms(), _t0) > 500:
                    print("SX1262 BUSY timeout in read()")
                    break
